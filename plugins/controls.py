...
async def set_vol(_, m: Message):
    if not Config.CALL_STATUS:
        ...
        return
    if len(m.command) < 2:
        await m.reply_text('Change Volume of Your Player. ㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤ', reply_markup=await volume_buttons())
        await delete_messages([m])
        return
...
async def settings(client, m: Message):
    await m.reply(f"Configure Your Player Settings Here. ㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤ", reply_markup=await settings_panel(), disable_web_page_preview=True)
    await delete_messages([m])
